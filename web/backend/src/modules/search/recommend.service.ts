...
export class RecommendService implements OnModuleInit {
	...
	constructor(
		...
		this.ollamaModel = this.configService.get<string>('ollama.model') || 'llama3.2';
	}
	...
	async getRecommendations(articleId: string, userId: string): Promise<RecommendedArticle[]> {
		...
		const sourceTags = sourceArticle.articleTags?.map(at => at.tag?.name).filter(Boolean) || [];
	...
	private async getTagBasedRecommendations(
		...
		const articleScores = new Map<string, { article: Article; matchingTags: string[]; score: number }>();
		...
		return sorted.map(s => {
			...
			const allTags = fullArticle?.articleTags?.map(at => at.tag?.name).filter(Boolean) || [];
	...
	private async enhanceWithAI(
		...
		try {
			...
			const prompt = this.buildAnalysisPrompt(sourceArticle, tagRecommendations);
	...
	private buildAnalysisPrompt(source: Article, recommendations: RecommendedArticle[]): string {
		const sourceTags = source.articleTags?.map(at => at.tag?.name).join(', ') || '';
