	...
	<div class="editor-view">
		<header class="editor-header">
			<div class="header-left">
				...
				<input v-model="title" class="title-input" placeholder="输入标题..." @input="markDirty" />
				<span v-if="isProdMode" class="mode-badge">Demo</span>
			</div>
			<div class="header-center">
				<TagSelector v-model="tagIds" placeholder="添加标签..." @update:modelValue="markDirty" />
			</div>
			<div class="header-right">
				...
				<button class="run-btn" :disabled="isRunning || !code.trim() || !canExecute"
					:title="!canExecute ? '当前模式不支持该语言执行' : ''" @click="runCode">
					{{ isRunning ? '运行中...' : '运行' }}
				</button>
				<button class="save-btn" :disabled="saving || !canSave" @click="saveArticle">
					...
					<div class="panel">
						<div class="panel-header">
							...
							<div class="panel-tabs">
								<button class="tab-btn" :class="{ active: mdViewMode === 'edit' }" @click="mdViewMode = 'edit'">
									编辑
								</button>
								<button class="tab-btn" :class="{ active: mdViewMode === 'preview' }" @click="mdViewMode = 'preview'">
									预览
								</button>
								<button class="tab-btn" :class="{ active: mdViewMode === 'split' }" @click="mdViewMode = 'split'">
									分屏
								</button>
							</div>
						</div>
						<div class="panel-content md-panel-content">
							...
							<div v-if="mdViewMode === 'edit'" class="md-edit-area">
								<MarkdownEditor v-model="content" placeholder="Write your thoughts here..."
					...
					<div class="right-panels">
						...
						<div class="panel code-panel" :style="{ height: codeHeight + '%' }">
							...
							<div class="panel-content">
								<CodeEditor v-model="code" :language="language" @update:modelValue="markDirty" @save="saveArticle"
									@run="runCode" />
							</div>
						</div>
						...
						<div class="panel preview-panel" :style="{ height: (100 - codeHeight) + '%' }">
							<HtmlPreview v-if="showHtmlPreview" :code="code" :language="language" />
							<CodePreview v-else :logs="executionLogs" :is-running="isRunning" @clear="clearLogs" />
						</div>
					</div>
				</template>
			</SplitPane>
