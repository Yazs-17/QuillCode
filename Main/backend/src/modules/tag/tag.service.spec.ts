...
describe('TagService', () => {
	...
	beforeEach(async () => {
		const module: TestingModule = await Test.createTestingModule({
			providers: [
				...
				{ provide: getRepositoryToken(ArticleTag), useValue: mockArticleTagRepository },
	...
	describe('create', () => {
		...
		it('should throw ConflictException if tag name exists for same user', async () => {
			mockTagRepository.findOne.mockResolvedValue({ id: 'existing', name: 'test-tag', userId });

			await expect(service.create(createTagDto, userId)).rejects.toThrow(ConflictException);
		});
	...
	describe('findAll', () => {
		...
		it('should return only tags belonging to the user', async () => {
			...
			mockArticleTagRepository.createQueryBuilder.mockReturnValue(mockQueryBuilder);
		...
		it('should return article counts scoped to user', async () => {
			...
			expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('article.userId = :userId', { userId });
	...
	describe('findOne', () => {
		...
		it('should throw NotFoundException if tag does not exist', async () => {
			...
			await expect(service.findOne(tagId, userId)).rejects.toThrow(NotFoundException);
		});
	...
	describe('remove', () => {
		...
		it('should throw NotFoundException when trying to delete another user\'s tag', async () => {
			...
			await expect(service.remove(tagId, userId)).rejects.toThrow(NotFoundException);
		});
	});
	...
	describe('addTagToArticle', () => {
		...
		it('should throw NotFoundException if tag does not belong to user', async () => {
			...
			await expect(service.addTagToArticle(articleId, tagId, userId)).rejects.toThrow(NotFoundException);
		});
		...
		it('should throw ForbiddenException if article does not belong to user', async () => {
			...
			await expect(service.addTagToArticle(articleId, tagId, userId)).rejects.toThrow(ForbiddenException);
		});
	});
});
